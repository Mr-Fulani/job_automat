...
class BrowserManager:
    ...
    async def get_context(self, use_session: bool = True) -> BrowserContext:
        ...
        if use_session:
            ...
            self._context = await self._browser.new_context(storage_state=str(self._settings.session_file))
        else:
    ...
    async def get_page(self, use_session: bool = True) -> AsyncGenerator[Page, None]:
        ...
        """
        try:
            context = await self.get_context(use_session=use_session)
